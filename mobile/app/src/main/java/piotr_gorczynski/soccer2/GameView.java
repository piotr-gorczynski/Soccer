...
public class GameView extends View {
    ...
    public void replaceMoves(ArrayList<MoveTo> newMoves) {
        Log.d("Soccer2", "GameView.replaceMoves called. newMoves.size=" + newMoves.size());
    ...
    public static class MyHandler extends Handler {
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            Log.d("Soccer2", "In handle message");
    ...
    public GameView(Context context, ArrayList<MoveTo> argMoves, int argGameType, String player0Name, String player1Name) {
        super(context);
        Log.d("Soccer2", "GameView Constructor called, received argMoves.size=" + argMoves.size());
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        super.onDraw(canvas);
        Log.d("Soccer2", "GameView.onDraw started");
    ...
    public boolean androidNextMove_v2(ArrayList<MoveTo> Moves,MoveTo masterMinMoveTo, int bouncingLevel, NextMoveFound masterNextMoveFound, int treeDepthLevel, long startThinkingTime) {
        ...
        Log.d("Soccer2", "<"+stringMove+" bouncinglevel='"+ bouncingLevel +"' treedepthlevel='"+ treeDepthLevel +"'>");
        logMoves(Moves);
        long difference;
        ...
        if(!possibleMoves.isEmpty()) {
            ...
            for (MoveTo i : possibleMoves) {
                ...
                if((nextMoveFound.found) && (nextMoveFound.bouncingLevel> gameBouncingLevel)) {
                    Log.d("Soccer2", "<gameBouncingLevelReached>" + gameBouncingLevel + "</gameBouncingLevelReached>");
                    break;
                }

                difference = (System.currentTimeMillis() - startThinkingTime)/1000;
                if((nextMoveFound.found) && (difference>androidLevel)) {
                    Log.d("Soccer2", "<timeLimitReached>" + difference + "</timeLimitReached>");
                    break;
                }
                ...
                if((checkWinner(i.X, i.Y, Moves) == 1) && (Moves.get(Moves.size() - 1).P==1)){
                    ...
                    Log.d("Soccer2", "<minimumfound1>" + minMoveTo + "</minimumfound1>");
                ...
                if (!(isBouncing(i.X, i.Y, Moves) || treeDepthLevel< gameTreeDepthLevel))
                {
                    if (((MINMAX(i.X, i.Y, Moves.get(Moves.size()-1).P) < MINMAX(minMoveTo.X, minMoveTo.Y, Moves.get(Moves.size()-1).P)) ) || (!nextMoveFound.found )) {
                        ...
                        while (bestMoves.size() > movesSize)
                            bestMoves.remove(bestMoves.size() - 1);
                        bestMoves.add(new MoveTo(minMoveTo.X, minMoveTo.Y, Moves.get(Moves.size() - 1).P));
                        Log.d("Soccer2", "<minimumfound player='"+ Moves.get(Moves.size() - 1).P +"' MINMAX='" + MINMAX(minMoveTo.X, minMoveTo.Y, Moves.get(Moves.size() - 1).P) +"'>" + minMoveTo + "</minimumfound>");
                        nextMoveFound.found = true;
                        nextMoveFound.bouncingLevel = bouncingLevel;
                        localNextMoveFound = true;
                        //if already landed in the gate
                        ...
                        if(checkWinner(i.X, i.Y, Moves) == 0) {
                            Log.d("Soccer2", "<defeat>" + stringMove + "</defeat>");
        ...
        Log.d("Soccer2", "<secondselapsed>"+ difference / 1000 +"</secondselapsed>");
        Log.d("Soccer2", "</"+stringMove+">");
    ...
    public void androidMove() {
        ...
        if (androidMoves.size() <= realMoves.size()) {
            Log.d("Soccer2", "GameView.androidMove In androidMove 1-st time");
            ...
            Log.d("Soccer2", "<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
            androidNextMove_v2(androidMoves, minMoveTo,0,nextMoveFound, 0, System.currentTimeMillis());
            if ( nextMoveFound.found ) {
                Log.d("Soccer2", "GameView.androidMove Sending message for androidMove 1-st time");
                mHandler.sendEmptyMessage(1);
            }
            else {
                gameActivity.showWinner(0);
            ...
            //called n-th time
            Log.d("Soccer2", "GameView.androidMove In androidMove n-th time");
            SystemClock.sleep(1000);
            boolean nextMovePossible = MakeMove(androidMoves.get(realMoves.size()).X, androidMoves.get(realMoves.size()).Y, realMoves);
            invalidate();
            if (nextMovePossible)
                if (realMoves.get(realMoves.size() - 1).P == 1) {
                    Log.d("Soccer2", "GameView.androidMove Sending message for androidMove n-th time");
                    mHandler.sendEmptyMessage(1);
                }
                else
                    androidMoves.clear();
        ...
        if (event.getAction()==MotionEvent.ACTION_UP)  {
            ...
            if(isMoveValid(x,y,possibleMoves)) {
                if (GameType == 3) {
                    ...
                    Log.d("Soccer2", "GameView.onTouchEvent Before invalidate, last P=" + realMoves.get(realMoves.size() - 1).P);
                    invalidate();
                    Log.d("Soccer2", "GameView.onTouchEvent After invalidate");
