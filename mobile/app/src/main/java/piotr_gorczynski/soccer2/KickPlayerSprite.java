...
/**
 * Loads kicking animation frames from the shared kick sprite sheet.
 */
final class KickPlayerSprite {

    private static final String TAG = "TAG_Soccer";

    static final long FRAME_DURATION_MS = 2000L;
    ...
    static synchronized Bitmap[] getFrames(Context context, int startY) {
        ...
        if (context == null) {
            Log.w(TAG, KickPlayerSprite.class.getSimpleName() + ".getFrames: context is null");
        ...
        if (spriteMetadata.getResourceId() == 0) {
            Log.w(TAG, KickPlayerSprite.class.getSimpleName() + ".getFrames: spritesheet_kick resource missing");
            Bitmap[] emptyFrames = new Bitmap[0];
            framesCache.put(startY, emptyFrames);
            return emptyFrames;
        }
